...
const fetchDirectories = async () => api.get("/config/directory");
...
export default function DirectoryProfiles() {
  ...
  const handleDeleteDirectory = async (id: number) => {
    if (confirm("Delete Directory?")) {
      try { await api.delete(`/config/directory/${id}`); refetch(); } catch (e: any) { alert(e.message); }
    }
  };
  ...
  const handleSaveEdit = async () => {
    ...
    if (!id) return;
    try {
      await api.put(`/config/directory/${id}`, editFormData());
      setEditingId(null);
      refetch();
    } catch (e: any) { alert(e.message); }
  };
      ...
      <div class="bg-[#111111] border border-zinc-800 rounded-lg shadow-sm overflow-hidden flex flex-col min-h-[500px]">
        <div class="p-4 border-b border-zinc-800 bg-[#161616]">
          <h2 class="text-sm font-semibold text-zinc-100 flex items-center gap-2">
            <FaSolidFolderTree class="text-purple-500" /> User Directories (AD/LDAP)
          </h2>
        </div>
        <div class="p-4 border-b border-zinc-800/80 bg-[#111111] grid grid-cols-1 md:grid-cols-5 gap-3">
          <input type="text" placeholder="Address (Host)" value={newDirHost()} onInput={e => setNewDirHost(e.currentTarget.value)} class="bg-[#161616] border border-zinc-700 rounded px-3 py-1.5 text-zinc-200 text-sm" />
          ...
          <input type="text" placeholder="Admin User" value={newDirUser()} onInput={e => setNewDirUser(e.currentTarget.value)} class="bg-[#161616] border border-zinc-700 rounded px-3 py-1.5 text-zinc-200 text-sm" />
          ...
          <button class="bg-purple-600 hover:bg-purple-500 text-white rounded text-sm font-medium flex items-center justify-center gap-2 transition-colors" onClick={handleAddDirectory}><FaSolidPlus /> Add Directory</button>
        </div>
        ...
        <div class="overflow-x-auto flex-1 bg-[#111111]">
          <table class="w-full text-left text-sm whitespace-nowrap">
            <thead class="text-xs text-zinc-500 uppercase bg-[#18181A] border-b border-zinc-800">
              <tr>
                <th class="px-5 py-3 font-medium">Address (Host)</th>
                <th class="px-5 py-3 font-medium">Port</th>
                <th class="px-5 py-3 font-medium">Admin User</th>
