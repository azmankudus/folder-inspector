...
export default function SqlBrowser() {
  ...
  const formatSqlValue = (val: any) => {
    if (val === null || val === undefined) return <span class="text-zinc-600 italic">NULL</span>;
    const s = String(val);
    // Simple check for ISO date-like strings (e.g., 2024-03-18T...)
      ...
      <div class="flex flex-col gap-6">
        ...
        <div class="bg-[#111111] border border-zinc-800 rounded-lg p-4 shadow-sm flex flex-col gap-4">
          <div class="flex items-center gap-4">
             <span class="text-sm font-semibold text-zinc-300 flex items-center gap-2">
                <FaSolidDatabase class="text-blue-500" /> Pick a Table:
             </span>
            <select 
          ...
          <div class="border-t border-zinc-800/80 pt-4">
            <label class="block text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-2">Manual Search</label>
        ...
        <div class="bg-[#111111] border border-zinc-800 rounded-lg shadow-sm overflow-hidden flex flex-col min-h-[400px]">
          <div class="p-4 border-b border-zinc-800 bg-[#161616]">
             <h2 class="text-sm font-semibold text-zinc-100">Search Results</h2>
          </div>
          
          <div class="overflow-x-auto flex-1 bg-[#111111]">
            <Show when={result()} fallback={
                <div class="text-zinc-600 flex items-center justify-center p-20 font-medium">Use the search box above to find data.</div>
            }>
               {(res) => (
