...
import { FaSolidClockRotateLeft, FaSolidSpinner, FaSolidTerminal, FaSolidTimes, FaSolidFileExport, FaSolidFileInvoice } from "solid-icons/fa";
...
export default function ScanHistoryView() {
  ...
  const [logs] = createResource(logModalOpen, async (job) => {
    try {
      return await api.get(`/jobs/${job.id}/db-logs`);
    } catch (e) {
      return [];
    }
  });
  ...
  const [exceptions] = createResource(exceptionModalOpen, async (job) => {
    try {
      return await api.get(`/jobs/${job.id}/exceptions`);
    } catch (e) {
      return [];
    }
  });
