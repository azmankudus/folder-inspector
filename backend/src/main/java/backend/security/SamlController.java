...
public class SamlController {
    ...
    private final org.pac4j.saml.client.SAML2Client client;
    ...
    public SamlController(
        ...
        org.pac4j.saml.config.SAML2Configuration config = new org.pac4j.saml.config.SAML2Configuration();
        ...
        this.client = new org.pac4j.saml.client.SAML2Client(config);
        this.client.init();
                ...
                .map(credentials -> {
                    return client.getUserProfile(ctx, credentials)
                            .map(profile -> {
                                ...
                                Authentication auth = new ServerAuthentication(username, Collections.singletonList("USER"), attributes);
                                
                                return tokenGenerator.generate(auth)
                                        .map(accessRefreshToken -> 
                                            HttpResponse.redirect(URI.create("http://localhost:3000/?token=" + accessRefreshToken.getAccessToken()))
                                        )
