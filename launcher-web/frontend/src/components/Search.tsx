...
const Search = (props: SearchProps) => {
  ...
  const inputClass = 'bg-white border-slate-300 text-slate-900 focus:ring-red-500 dark:bg-slate-700 dark:border-slate-600 dark:text-white dark:placeholder-slate-400';
  ...
  const columns: Column<FileEntry>[] = [
    ...
    { key: 'size', label: 'Size', sortable: true, filterType: 'size-range', width: 'w-[180px]', render: (row) => <span class="font-mono text-xs text-slate-600 dark:text-slate-400">{formatBytes(row.size)}</span> },
    { key: 'modified', label: 'Modified', sortable: true, filterType: 'datetime-range', width: 'w-[160px]', render: (row) => <span class="text-xs opacity-80 whitespace-nowrap">{new Date(row.modified).toLocaleString(undefined, { timeZoneName: 'short' })}</span> },
    {
    ...
    },
    {
      ...
      render: (row) => (
        <button
          ...
          class="w-8 h-8 flex items-center justify-center rounded-full bg-blue-100 hover:bg-blue-200 text-blue-700 transition-colors"
          title="View Details"
        >
  ...
  return (
    <div class="space-y-6">
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <h2 class="text-2xl font-bold text-slate-800 dark:text-white">Search</h2>
        ...
        <div class="flex items-center gap-2 w-full md:w-auto">
          ...
          <Show when={selectedConfig()}>
            <button
              ...
              class="w-10 h-10 flex items-center justify-center rounded-full transition-colors shrink-0 bg-gradient-to-br from-red-600 to-slate-900 text-white hover:shadow-md"
      ...
      <Modal
        ...
        title="Configuration Details"
        icon={<i class="fa-solid fa-file-lines text-lg text-blue-700 dark:text-blue-400"></i>}
      >
        <Show when={selectedConfig()}>
          <div class="space-y-4">
            ...
            <div class="flex justify-end pt-4 mt-2 border-t border-slate-200 dark:border-slate-700">
              <button onClick={() => setShowConfigDetails(false)} class="px-4 py-2 text-sm font-medium text-white bg-gradient-to-br from-red-600 to-slate-900 rounded-lg shadow-sm hover:shadow-md transition-colors">Close</button>
            </div>
          </div>
        </Show>
      </Modal>
