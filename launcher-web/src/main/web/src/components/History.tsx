...
const History: Component<HistoryProps> = (props) => {
    ...
    },
    {
      ...
      render: (row) => (
        <Show when={row.status === 'In Progress'}>
          <button onClick={() => setStoppingScanId(row.id)} class="w-8 h-8 flex items-center justify-center rounded-full bg-red-100 hover:bg-red-200 text-red-600 transition-colors" title="Stop Scan">
  ...
  return (
    <div class="space-y-6">
      <h2 class="text-2xl font-bold text-slate-800 dark:text-white">Scan History</h2>
      ...
      >
        <div class="text-center p-2">
          ...
          <div class="flex justify-center gap-3">
            ...
            <button onClick={confirmStopScan} class="px-4 py-2 text-sm font-medium text-white bg-gradient-to-br from-red-600 to-slate-900 rounded-lg shadow-sm hover:shadow-md transition-all">Yes</button>
          </div>
        </div>
      </Modal>
    </div>
