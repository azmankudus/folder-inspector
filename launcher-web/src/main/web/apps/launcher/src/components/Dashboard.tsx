...
const Dashboard = (props: DashboardProps) => {
  ...
  const cardClass = 'bg-white border-slate-200 text-slate-800 dark:bg-slate-800 dark:border-slate-700 dark:text-white';
    ...
    <div class="space-y-6">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
          ...
          <For each={stats}>
            {stat => {
              const colorMap: Record<string, { text: string; bg: string }> = {
                red: { text: 'text-red-600 dark:text-red-400', bg: 'bg-red-50 text-red-600 dark:bg-red-900/30 dark:text-red-400' },
                blue: { text: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400' },
                amber: { text: 'text-amber-500 dark:text-amber-400', bg: 'bg-amber-50 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400' },
              };
              ...
              return (
                <div class={`p-5 rounded-xl border shadow-sm ${cardClass} ${stat.label === 'Total Owned' ? 'sm:col-span-2' : ''} flex flex-col justify-between`}>
                  ...
                  <div class="mt-6 w-full h-2 rounded-full overflow-hidden bg-slate-100 dark:bg-slate-700">
        ...
        <div class={`p-5 rounded-xl border shadow-sm ${cardClass} flex flex-col h-full`}>
          <h3 class="font-semibold text-lg mb-4 text-slate-900 dark:text-white">Access Rights Distribution</h3>
          <div class="flex-1 flex flex-col lg:flex-row items-center justify-center gap-8 pie-container relative" style={{ "min-height": "300px" }}>
                  ...
                  return (
                    <div class="flex items-center gap-3">
                      ...
                      <div class="flex flex-col">
                        <span class="text-sm font-medium text-slate-700 dark:text-slate-200">{d.name}</span>
                        <span class="text-xs text-slate-500 dark:text-slate-400">{d.value} items · {pct}%</span>
                      </div>
                    </div>
                  );
                }}
            ...
            <Show when={tooltip()}>
              {t => (
                <div class="absolute z-50 px-4 py-2.5 rounded-lg shadow-xl text-sm pointer-events-none bg-white text-slate-800 border border-slate-200 dark:bg-slate-700 dark:text-white dark:border-slate-600"
                  style={{ left: `${t().x}px`, top: `${t().y - 50}px`, transform: "translateX(-50%)" }}>
                  <div class="font-bold">{t().name}</div>
                  <div class="text-slate-500 dark:text-slate-300">{t().value} items · {t().pct}%</div>
                </div>
              )}
            </Show>
          </div>
      ...
      <div>
        <h3 class="font-semibold text-lg mb-4 text-slate-900 dark:text-white">Files per Configuration</h3>
            ...
            {(item, i) => {
              ...
              return (
                <div class="rounded-xl border shadow-sm flex flex-col items-center justify-center py-8 px-4 transition-transform hover:scale-105"
                  ...
                  <span class="text-sm font-medium mt-3 text-center text-slate-600 dark:text-slate-300">{item.name}</span>
                </div>
              );
            }}
          </For>
