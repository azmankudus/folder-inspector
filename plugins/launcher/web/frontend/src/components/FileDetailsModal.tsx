...
const FileDetailsModal: Component<FileDetailsModalProps> = (props) => {
  ...
  return (
    ...
    >
        ...
        <div class="space-y-4">
          ...
          <div class={`rounded-lg border overflow-hidden ${borderClass()}`}>
            <table class={`w-full text-sm text-left ${textClass()}`}>
              <thead class={`text-xs uppercase font-bold ${isDark() ? 'bg-slate-900/50 text-slate-400' : 'bg-slate-100 text-slate-500'}`}>
                <tr>
              ...
              <tbody class={`divide-y ${borderClass()}`}>
                <Show when={props.file.acl && props.file.acl.length > 0} fallback={<tr><td colspan="3" class="px-3 py-4 text-center text-slate-500 italic">No ACL entries found.</td></tr>}>
                  <For each={props.file.acl}>{acl => (
                    <tr class={isDark() ? 'hover:bg-slate-700/50' : 'hover:bg-slate-50'}>
      ...
      <div class={`flex justify-start mt-6 pt-4 border-t ${borderClass()}`}>
        <button onClick={props.onClose} class="px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 hover:bg-slate-50 rounded-lg shadow-sm transition-colors dark:bg-slate-800 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-700">Close</button>
      </div>
    </Modal>
  );
};
